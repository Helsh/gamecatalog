...
class WebHelper:
    def generateRandomClientId(self, rndNumb):
        # Generate random chain of strings and digits with length of 64 signs
        # which will be our antiforgery state
        state = ''.join(random.choice(
            string.ascii_uppercase + string.digits) for x in range(rndNumb))
        return state
